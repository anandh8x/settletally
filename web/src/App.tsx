  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <a className="brand" href="#top" aria-label="SettleTally home">
      ...
      <main id="top">
        ...
        <section className="workspace" id="workspace">
          <div className="section-intro">
            ...
            <h2>Expected records in.<br />Exceptions out.</h2>
            <p>Import the records your books expect, choose the Arc account and block range, then compare them with settled USDC transfers.</p>
          </div>

          <div className="work-grid">
            <div className="panel import-panel">
              <div className="panel-title"><span>1</span><div><h3>Expected records</h3><p>Invoices, receivables, or payouts in CSV format.</p></div></div>
              <div
              ...
              >
                <input
                  ...
                  type="file"
                  accept=".csv,text/csv"
                  aria-label="Import expected records CSV"
                  onChange={(event) => {
                ...
                {records.length ? (
                  <><strong>{fileName}</strong><p>{records.length} records ready · {expectedTotal.toLocaleString("en-US", { maximumFractionDigits: 6 })} USDC expected</p></>
                ) : (
                  <><strong>Drop your CSV here</strong><p>or select a file from your computer</p></>
                )}
                <button type="button" onClick={() => inputRef.current?.click()}>{records.length ? "Replace file" : "Choose CSV"}</button>
              </div>
              <button className="template-link" type="button" onClick={downloadTemplate}>Download the CSV template ↓</button>
              {records.length > 0 && <RecordPreview records={records} />}
            </div>

            <div className="panel account-panel">
              <div className="panel-title"><span>2</span><div><h3>Arc activity</h3><p>The account and history to compare.</p></div></div>
              <label className="field">
                <span>ARC WALLET ADDRESS</span>
                <input value={wallet} onChange={(event) => setWallet(event.target.value)} placeholder="0x..." spellCheck={false} />
              </label>
              ...
              <div className="scope-note"><span aria-hidden="true">◇</span><p><strong>Read-only by design</strong>SettleTally reads public Arc events. It never requests a signature or private key.</p></div>
        ...
        <section className="method" id="method">
          <div className="section-intro compact"><p className="eyebrow">DETERMINISTIC BY DESIGN</p><h2>A result you can inspect.</h2></div>
          <div className="method-grid">
            <article><span>01</span><h3>Read settled activity</h3><p>Canonical USDC Transfer events, Arc transaction memos, block positions, and paid gas fees are read from Arc.</p></article>
            <article><span>02</span><h3>Apply ordered rules</h3><p>Memo IDs and references take priority, followed by exact counterparty and amount checks. Weak candidates are never silently accepted.</p></article>
            <article><span>03</span><h3>Expose exceptions</h3><p>Partial, excess, missing, duplicate, and unmatched payments remain visible with the specific reason behind each status.</p></article>
          </div>
        </section>
      </main>

      <footer><a className="brand footer-brand" href="#top"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span>SettleTally</span></a><p>Working Arc Testnet product · Public onchain data remains public</p><a href="https://testnet.arcscan.app" target="_blank" rel="noreferrer">Open ArcScan ↗</a></footer>
    </div>
  );
}

function RecordPreview({ records }: { records: ExpectedInput[] }) {
  return <div className="record-preview"><div className="preview-head"><span>REFERENCE</span><span>DIRECTION</span><span>AMOUNT</span></div>{records.slice(0, 4).map((record) => <div className="preview-row" key={record.reference}><strong>{record.reference}</strong><span>{record.direction}</span><span>{record.amount} USDC</span></div>)}{records.length > 4 && <p>+ {records.length - 4} more records</p>}</div>;
}
